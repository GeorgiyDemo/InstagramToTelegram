...
class Redis:
    ...
    def get_data(self):
        ...
        lst = []
        for value in self.r_table.values():
            lst.append(value)
        return lst
...
class Instagram2Telegram:
    ...
    def processing(self):
        ...
        if media_id not in media_ids:
            ...
            username = self.getter(username_url+code)["entry_data"]["PostPage"][0]["graphql"]["shortcode_media"]["owner"]["username"]
            
            # Добавляем в Redis
            self.r_obj.data = {media_id : username}

            #Кидаем пост в Telegram
            send_telegram(tag_page, username)
    ...
    def send_telegram(self, media_json, username):
        ...
        if media_json["is_video"]:
            ...
            buf_data = getter(username_url+media_json["shortcode"])
